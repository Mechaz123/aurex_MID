...
export class ProductoService {
    ...
    async getProductosVentaPropietario(id: string, token: string) {
    ...
    async getProductosIntercambioPropietario(id: string, token: string) {
        const headers = { Authorization: `Bearer ${token}`};
        const todosProductos = await this.utilsService.SendGet<Producto[]>(process.env.AUREX_MID_AUREX_CRUD_URL, "producto", headers);
    ...
    async getProductosSubastaPropietario(id: string, token: string) {
        const headers = { Authorization: `Bearer ${token}`};
        const todosProductos = await this.utilsService.SendGet<Producto[]>(process.env.AUREX_MID_AUREX_CRUD_URL, "producto", headers);
    ...
    async getProductosDonacionPropietario(id: string, token: string) {
        const headers = { Authorization: `Bearer ${token}`};
        const todosProductos = await this.utilsService.SendGet<Producto[]>(process.env.AUREX_MID_AUREX_CRUD_URL, "producto", headers);
